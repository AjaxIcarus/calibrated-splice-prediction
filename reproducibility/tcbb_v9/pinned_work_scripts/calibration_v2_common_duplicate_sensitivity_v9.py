#!/usr/bin/env python3
"""Strict TCBB v9 validator for the duplicate-exclusion sensitivity cache."""
...
def _validate_metadata(metadata: dict[str, Any], path: Path) -> None:
    expected_flags = {
        ...
        "all_sequences_included": 0,
        "sensitivity_exclusion_applied": 1,
    ...
    expected = {
        ...
        "negatives_seen_total": 88982074,
        "sampled_negatives": 500000,
        "random_seed": 11,
        "total_shards_seen": 14,
        "max_negatives": 500000,
        "excluded_sequence_count": 3,
    }
    for name, wanted in expected.items():
        actual = int(metadata[name])
        if actual != wanted:
            raise RuntimeError(
                f"{path}: {name} must be {wanted}; found {actual}"
            )
...
def load_and_validate_cache(path: str | Path) -> CacheData:
    ...
    with np.load(cache_path, allow_pickle=False) as cache:
        ...
        excluded_shard_indices = np.asarray(
            cache["excluded_shard_indices"], dtype=np.int64
        ).reshape(-1).copy()
        excluded_row_indices = np.asarray(
            cache["excluded_row_indices"], dtype=np.int64
        ).reshape(-1).copy()
    ...
    expected_shards = np.array([2, 11, 12], dtype=np.int64)
    expected_rows = np.array([651, 900, 518], dtype=np.int64)

    if not np.array_equal(excluded_shard_indices, expected_shards):
        raise RuntimeError(
            f"{cache_path}: excluded_shard_indices must be "
            f"{expected_shards.tolist()}; found "
            f"{excluded_shard_indices.tolist()}"
        )
    if not np.array_equal(excluded_row_indices, expected_rows):
        raise RuntimeError(
            f"{cache_path}: excluded_row_indices must be "
            f"{expected_rows.tolist()}; found "
            f"{excluded_row_indices.tolist()}"
        )

    expected_class_counts = np.array(
        [500000, 12951, 12951],
        dtype=np.int64,
    )
    if not np.array_equal(class_counts, expected_class_counts):
        raise RuntimeError(
            f"{cache_path}: sampled class counts must be "
            f"{expected_class_counts.tolist()}; found "
    ...
    if not dataset_path.is_file():
        raise RuntimeError(
            f"{cache_path}: recorded dataset_path does not exist: "
            f"{dataset_path}"
        )

    expected_dataset_sha256 = (
        "bafc1f0352a392d6552ddb53f9792fb29b16a5312b5e61105d73044652e3343e"
    )
    dataset_sha256 = sha256_file(dataset_path)
    if dataset_sha256 != expected_dataset_sha256:
        raise RuntimeError(
            f"{cache_path}: validation dataset SHA-256 mismatch: "
            f"{dataset_sha256}"
        )

    metadata = dict(metadata)
    metadata["dataset_path"] = str(dataset_path)
    metadata["dataset_sha256_verified"] = dataset_sha256
