...
def valid_epoch(model, h5f, idxs, batch_size, criterion, device, params, metric_files, flanking_size, run_mode):
    ...
    for i, shard_idx in enumerate(shuffled_idxs, 1):
        ...
        for batch in pbar:
            ...
            # Low-memory patch:
            # During full training, do not accumulate every prediction/label tensor.
            # The full train set is large, and storing all of this can exhaust RAM.
            # Validation/test still keep the original metric behavior.
...
def train_epoch(model, h5f, idxs, batch_size, criterion, optimizer, scheduler, device, params, metric_files, flanking_size, run_mode, global_batch_idx):
    ...
    for i, shard_idx in enumerate(shuffled_idxs, 1):
        ...
        pbar.close()
    # Low-memory patch:
    # For training, return average batch loss directly instead of computing
    # full-epoch train metrics from accumulated predictions/labels.
    if run_mode.lower() == "train":
        avg_train_loss = running_loss / max(train_batches_seen, 1)
        return torch.tensor(avg_train_loss), global_batch_idx
...
def train_model(model, optimizer, scheduler, train_h5f, valid_h5f, test_h5f, train_idxs, val_idxs, test_idxs,
    ...
    for epoch in range(args.epochs):
        ...
        # Low-memory safety checkpoint:
        # Save immediately after training, before validation/test metrics.
        torch.save(model.state_dict(), f"{model_output_base}/model_{epoch}_after_train.pt")
        print(f"Saved after-train checkpoint: {model_output_base}/model_{epoch}_after_train.pt")
        ...
        # Low-memory patch:
        # Skip duplicate test evaluation during training/debug runs.
        # We are passing validation as test anyway, so this avoids a second full eval pass.
        test_loss = val_loss

        print(f"Training Loss: {train_loss}")
        print(f"Validation Loss: {val_loss}")
        print(f"Testing Loss: {test_loss}  [skipped duplicate test pass; using validation loss]")
