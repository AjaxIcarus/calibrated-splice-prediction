...
def train(args):
    ...
    # Resume/checkpoint loading patch:
    # The original train CLI accepts --model, but does not load checkpoint weights.
    # If --model points to a .pt file, load it before continuing training.
    if args.model != "SpliceAI" and os.path.isfile(args.model):
        print(f"Loading model checkpoint from: {args.model}")
        state_dict = torch.load(args.model, map_location=device)
        model.load_state_dict(state_dict)
        print("Checkpoint loaded successfully.")
    elif args.model != "SpliceAI":
        print(f"WARNING: --model was provided but is not a file: {args.model}")
