...
def iter_complete_shard_batches(h5f, shard_idx, batch_size):
    ...
    for start in range(0, sequence_count, batch_size):
        ...
        dnas_np = np.asarray(x_ds[start:stop]).transpose(0, 2, 1)
    ...
    with h5py.File(dataset_path, "r") as h5f:
        ...
        with torch.no_grad():
            for shard_idx in idxs:
                ...
                if shard_sequences_seen != shard_sequence_count:
                    raise RuntimeError(
                ...
                print(
                    f"Shard {shard_idx}: retained all "
                    f"{shard_sequences_seen:,} sequences",
                    flush=True,
                )
