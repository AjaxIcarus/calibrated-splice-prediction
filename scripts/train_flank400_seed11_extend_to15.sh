...
for epoch in range(1, current + 1):
    log = root / "logs" / f"train_flank400_focal_epoch{epoch}.log"
...
for epoch in $(seq 10 15); do
    ...
    previous_checkpoint="results/best_models/flank400_focal_epoch${previous_epoch}_best.pt"
    checkpoint="results/best_models/flank400_focal_epoch${epoch}_best.pt"
    outdir="results/full_train_flank400_focal_epoch${epoch}_lowmem"
    project="flank400_full_focal_epoch${epoch}_lowmem"
    log="logs/train_flank400_focal_epoch${epoch}.log"
    ...
    if [[ -f "$checkpoint" ]]; then
        ...
        echo "Starting seed 11, epoch ${epoch}"
...
for epoch in range(1, 16):
    ...
    checkpoint = (
        ...
        / f"flank400_focal_epoch{epoch}_best.pt"
    )
...
table = root / "results/flank400_seed11_validation_selection.csv"
...
alias = root / "results/best_models/flank400_focal_best.pt"
shutil.copy2(source, alias)

digest = hashlib.sha256(alias.read_bytes()).hexdigest()

selection = root / "results/best_models/flank400_seed11_selection.txt"
